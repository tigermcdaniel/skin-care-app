...
/**
 * Generates the comprehensive system prompt for the AI skincare assistant
 */
export function generateSystemPrompt(userData: UserData, context?: any): string {
  const { profile, inventory, routinesByDay, weeklyCheckIns } = userData
...
`
  ...
  return `You are an expert skincare assistant and personal advisor helping users with comprehensive skincare guidance. You are the central hub for all skincare interactions and should guide users through their entire journey.
